...
@section('content')
    ...
    <div class="site-hero w-full relative">
        @if (!isset($setting->banner_image))
            <img class="w-full object-cover bg-cover msm:block" src="{{ url('images/upload/' . $setting->banner_image) }}"
                alt="">
        @else
            <img class="w-full object-cover bg-cover msm:block" src="{{ url('images/upload_empty/hero.png') }}"
                alt="">
        @endif
        <div class="btn-appointment mx-0  mt-3  absolute xxsm:relative xsm:relative sm:absolute">
            <a class="btn btn-link text-center mt-0 rounded-none bg-primary text-white font-normal font-fira-sans text-sm py-3.5 px-7"
                target="_blank" href="{{ $setting->banner_url }}" role="button">{{ __('Make Appointment') }}</a>
        </div>
    </div>
